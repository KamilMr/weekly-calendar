...
const cal = utils.createDiv({className: 'container', styles: {
}});
const wrapper = utils.createDiv({
  className: 'container', 
  styles: {
    display: 'flex',
...
// Create day headers
const dayHeaders = Array.from({length: NUM_OF_COL})
  .map((_, idx) => utils.createElement({
    id: `header${idx+1}`,
    tag: 'div',
    textContent: `Day ${idx+1}`,
    className:'box',
    styles: {
      textAlign: 'center',
      fontWeight: 'bold',
...
const textElements = Array.from({length: NUM_OF_COL})
  .map(el => utils.createElement({tag: 'p'}))
  .map((tag, idx) => {
    tag.innerHTML = `container ${idx + 1}`
  return tag;
});

// dayHeaders.forEach((el,idx) => utils.appendElement(el, textElements[idx]))

const columns = Array.from({length: NUM_OF_COL})
  .map((_, idx) => utils.createDiv({className: `box box${idx+1}`, id: `column${idx+1}`}));

// append headers and cols and events
// dayHeaders.forEach(el => utils.appendElement(headers, el));
// columns.forEach(el => utils.appendElement(cal, el))
// allEvents.forEach(el => utils.appendElement(cal, el))

// utils.appendElement(wrapper, headers)
// utils.appendElement(wrapper, cal)
// utils.appendElement(root, wrapper);
...
export function createEngine({cols}) {
  let host = null;
  const listeners = {};
  const attach = el => {host = el};
  const destroy = () => host = null;
  return {
    listeners,
    attach, 
    destroy,
